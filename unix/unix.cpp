...
uint32 S9xReadJoypad (int which1)
{
	...
	if (keyssnes[sfc_key[X_1]] == SDL_PRESSED || joy_buttons[which1][JB_X])		val |= SNES_X_MASK;
	if (keyssnes[sfc_key[Y_1]] == SDL_PRESSED || joy_buttons[which1][JB_Y])		val |= SNES_Y_MASK;
	if (keyssnes[sfc_key[B_1]] == SDL_PRESSED || joy_buttons[which1][JB_B])		val |= SNES_B_MASK;
	if (keyssnes[sfc_key[A_1]] == SDL_PRESSED || joy_buttons[which1][JB_A])		val |= SNES_A_MASK;
	if (keyssnes[sfc_key[START_1]] == SDL_PRESSED || joy_buttons[which1][JB_START])	val |= SNES_START_MASK;
	if (keyssnes[sfc_key[SELECT_1]] == SDL_PRESSED || joy_buttons[which1][JB_SELECT])	val |= SNES_SELECT_MASK;
